...
PORT = '8080', POLL_INTERVAL_MS = '15000', } = process.env;
...
async function reconcileStatus(row) {
    ...
    const readChain = !!row.pool_address && (row.status === 'upcoming' || row.status === 'active' || row.status === 'ended');
...
async function checkFinalizeEligibility(pool) {
    ...
    const canFinalize = (!s.finalized && !s.failed) &&
...
async function callFinalize(pool) {
    ...
    if (!st.canFinalize && afterEnd && belowSoft && !st.finalized && !st.failed) {
        log.info({ pool, totalRaised: st.totalRaised.toString(), softCap: st.softCap.toString(), endAt: st.endAt.toString(), now: st.now }, 'preflight: mark failed (below soft cap after end)');
...
function startRealtime() {
    const ch = supabase
        ...
        try {
            ...
            if (reconciled === 'ended' && next.finalized === false && next.finalizing === false) {
...
async function pollOnce() {
    ...
    for (const r of data ?? []) {
        ...
        try {
            ...
            if (next === 'ended' && !row.finalized && !row.finalizing) {
